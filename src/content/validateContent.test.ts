...
describe('basicEnglishCourse V1.12', () => {
  ...
  it('includes complete Week 5 and Week 6 course content for V1.10', () => {
    ...
    expect(result.errors).toEqual([]);
    expect(basicEnglishCourse.contentVersion).toBe('1.13.0');
  ...
  it('includes complete Week 8 through Week 12 course content for V1.12', () => {
    ...
    expect(result.errors).toEqual([]);
    expect(basicEnglishCourse.contentVersion).toBe('1.13.0');
    expect(basicEnglishCourse.weeks).toHaveLength(13);
    expect(basicEnglishCourse.weeks.map((week) => week.days.length)).toEqual([7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7]);
  ...
  it('keeps Day 91 as the course completion day', () => {
    const allDays = basicEnglishCourse.weeks.flatMap((week) => week.days);

    expect(allDays.at(-1)?.id).toBe('day-091');
